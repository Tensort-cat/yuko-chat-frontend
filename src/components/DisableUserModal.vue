  ...
  <div style="height: 100%; width: 100%" v-if="isVisible">
    ...
    <div class="main-able-button-container">
      <el-button style="background-color: rgb(252, 210.9, 210.9); margin-left: 20px"
        @click="disableUsers">禁用/全部禁用</el-button>
      <el-button style="background-color: rgb(252, 210.9, 210.9)" @click="ableUsers">启用/全部启用</el-button>
    </div>
  </div>
</template>
    ...
    const ableUsers = async () => {
      try {
        const req = {
          ...
          is_admin: store.state.userInfo.is_admin,
        }
        const rsp = await axios.post(
